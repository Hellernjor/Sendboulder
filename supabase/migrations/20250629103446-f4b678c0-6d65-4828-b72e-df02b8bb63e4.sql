...
CREATE POLICY "Grade levels are viewable by everyone" ON grade_levels FOR SELECT USING (true);
CREATE POLICY "Location creators can manage grade levels" ON grade_levels FOR ALL USING (
  EXISTS (
...
CREATE POLICY "Users can update their own routes" ON routes FOR UPDATE USING (auth.uid() = created_by);
CREATE POLICY "Users can delete their own routes" ON routes FOR DELETE USING (auth.uid() = created_by);
...
CREATE POLICY "Users can delete their own attempts" ON attempts FOR DELETE USING (auth.uid() = user_id);
